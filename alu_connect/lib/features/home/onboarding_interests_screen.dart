              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Select topics to personalize your ALU Connect+ feed and community recommendations.',
                        style: TextStyle(color: Color(0xFF8A8D99), fontSize: 13, height: 1.55)),
                    const SizedBox(height: 28),
                    ..._groups.map((g) => _InterestGroup(
                        icon: g['icon'], label: g['label'],
                        topics: g['topics'], selected: _selected, onToggle: _toggle)),
...
class _InterestGroup extends StatelessWidget {
  ...
  const _InterestGroup({required this.icon, required this.label,
      required this.topics, required this.selected, required this.onToggle});
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(label, style: const TextStyle(color: Color(0xFFF0EDE4), fontSize: 13, fontWeight: FontWeight.w500)),
        ]),
        const SizedBox(height: 10),
        Wrap(spacing: 8, runSpacing: 8,
          children: topics.map((t) {
            final sel = selected.contains(t);
            return GestureDetector(onTap: () => onToggle(t),
              child: AnimatedContainer(duration: const Duration(milliseconds: 180),
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: sel
                      ? const Color(0xFFF5C842).withOpacity(0.6) : Colors.white.withOpacity(0.08)),
                ),
